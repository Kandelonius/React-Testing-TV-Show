...
const episodeData = {
    data: {
        ...
        //     imdb: 'tt4574334'
        // },
        image: {// need this property for the div being created in the app function.
...
test("renders without errors", () => {
    mockFetchShow.mockResolvedValue(episodeData)// using mockResolvedValue omitting the once allows me to use this again in the next test.
    render(<App />);// getting cannot read property .then of undefined pointing at this line???
});

test("renders data after API is called", async () => {
    // mockFetchShow.mockResolvedValueOnce(episodeData)// to use the results of the api call without having to call it again.
    const { getByText, queryAllByTestId } = render(<App />)
    await waitFor(() => {// wait
         userEvent.click(getByText(/select a season/i))// click the dropdown for season selection
         userEvent.click(getByText(/season 1/i))// select season 1
